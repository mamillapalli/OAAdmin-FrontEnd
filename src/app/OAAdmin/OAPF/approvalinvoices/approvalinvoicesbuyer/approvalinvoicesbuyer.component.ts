...
export class ApprovalinvoicesbuyerComponent implements OnInit {
  ...
  public getInvoices() {
    ...
    const sb = this.oapfcommonService.getDataWithPagination('/oapf/api/v1/invoices/forAnchorPartyApproval', this.currentPage, this.pageSize, this.sortData).subscribe((res) => {
  ...
  openFilter() {
    ...
    modalRef.result.then((result) => {
      ...
      if (result.valid && result.value.filterOption.length > 0) {
        const sb = this.oapfcommonService.getFilterWithPagination(result, 'filter', 'oapf/api/v1/invoices/forAnchorPartyApproval',this.currentPage,this.pageSize,this.sortData).subscribe((res) => {
          this.dataSource.data = res.content;
          this.totalRows = res.totalElements
        });
        this.subscriptions.push(sb);
      } else {
        const sb = this.oapfcommonService.getFilterWithPagination(result, 'all', 'oapf/api/v1/invoices/forAnchorPartyApproval',this.currentPage,this.pageSize,this.sortData).subscribe((res) => {
          this.dataSource.data = res.content;
          this.totalRows = res.totalElements
        });
        this.subscriptions.push(sb);
