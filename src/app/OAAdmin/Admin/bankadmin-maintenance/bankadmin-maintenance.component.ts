...
export class BankadminMaintenanceComponent implements OnInit {
  ...
  public applyFilter(event: any,label:any) {
    ...
    if(this.searchFilter.values !== null) {
      ...
      const sb = this.oaCommonService.getFilterWithPagination(htp, 'filterByData', '/oaadmin/api/v1/bankadmins', this.currentPage, this.pageSize, this.sortData).subscribe((res: any) => {
        this.dataSource.data = res.content;
        this.totalRows = res.totalElements
      });
      this.subscriptions.push(sb);
