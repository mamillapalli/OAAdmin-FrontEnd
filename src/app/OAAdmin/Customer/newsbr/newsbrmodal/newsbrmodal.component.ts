...
import { NewsbrmainComponent } from './newsbrmain/newsbrmain.component';
import { NewsbramtinfoComponent } from './newsbramtinfo/newsbramtinfo.component';

@Component({
  selector: 'app-newsbrmodal',
  templateUrl: './newsbrmodal.component.html',
  styleUrls: ['./newsbrmodal.component.scss']
})
export class NewsbrmodalComponent implements OnInit {
  ...
  @ViewChild(NewsbrmainComponent) NewsbrmainComponent: NewsbrmainComponent;
  @ViewChild(NewsbramtinfoComponent) NewsbramtinfoComponent: NewsbramtinfoComponent;
  closeResult: string;
    ...
    if (this.currentStep$.value === this.formsCount - 1) {
     ...
     //  this.Sbrreq.agreement=this.account$.value.agreement
      ...
      if (this.mode === 'new') {
        ...
        this.oapfcommonService.dataItem(this.Sbrreq, '', this.mode,'oaadmin/api/v1/sbrs').subscribe(res => {
          if (res !== undefined) {
  ...
  copyAs() {
    ...
    modalRef.result.then((result) => {
      const refNo = this.NewsbrmainComponent.form.value.sbrId;
      ...
      this.NewsbrmainComponent.form.patchValue(this.formValue)
      this.NewsbrmainComponent.form.patchValue(this.formValue)
      this.NewsbrmainComponent.form.value.sbrId = refNo;
